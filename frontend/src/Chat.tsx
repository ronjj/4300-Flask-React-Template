...
function Chat({ onSearchTerm }: ChatProps): JSX.Element {
  ...
  const sendMessage = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText =
          data.answer || data.error || `Error: ${response.status}`;
        setMessages((prev) => [
          ...prev,
          { text: String(errorText), isUser: false },
        ]);
        return;
      }
      ...
      if (!answer) {
        const fallbackError =
          data.error || "Error: Invalid response from server";
        setMessages((prev) => [
          ...prev,
          { text: String(fallbackError), isUser: false },
        ]);
        return;
      }
      ...
      const rewrittenQuery =
        typeof data.rewritten_query === "string" ? data.rewritten_query : null;
      const retrievalConfidence =
        typeof data.retrieval_confidence === "number"
          ...
          >
            {!msg.isUser && msg.rewrittenQuery && (
              <div className="query-expansion-banner">
                <span className="query-expansion-label">LLM-EDITED QUERY</span>
                  ...
                  <div className="chat-meta">
                    {msg.retrievalConfidence != null && (
                      <p className="chat-meta-line">
                        ...
                        {msg.retrievalConfidence.toFixed(3)}
                      </p>
                    )}
                    {(msg.results?.length ?? 0) > 0 && (
                        ...
                        <ul className="chat-meta-list">
                          {msg.results
                            ...
                            .map((result, resultIndex) => (
                              <li
                                key={`${result.player_id ?? result.player_name ?? "result"}-${resultIndex}`}
                              >
                        ...
                        <ul className="chat-meta-list">
                          {msg.evidence
                            ...
                            .map((item, evidenceIndex) => {
                              ...
                              return (
                                <li
                                  key={`${item.evidence_id ?? item.player_name ?? "evidence"}-${evidenceIndex}`}
                                >
